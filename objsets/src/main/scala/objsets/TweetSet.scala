...
abstract class TweetSet {
  ...
  /** This method takes a predicate and returns a subset of all the elements in
    ...
    */
  def filter(pred: Tweet => Boolean): TweetSet = filterAcc(pred, new Empty)

  /** This is a helper method for `filter` that propagetes the accumulated
    * tweets.
    */
  def filterAcc(pred: Tweet => Boolean, accum: TweetSet): TweetSet

  /** Returns a new `TweetSet` that is the union of `TweetSet`s `this` and
    * `that`.
    *
    * Question: Should we implment this method here, or should it remain
  ...
  /** Returns a list containing all tweets of this set, sorted by retweet count
    ...
    * Should we implment this method here, or should it remain abstract and be
  ...
  /** Tests if this set is empty or not.
    */
  def isEmpty: Boolean
}
...
class Empty extends TweetSet {
  ...
  def filterAcc(pred: Tweet => Boolean, accum: TweetSet): TweetSet = accum
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(pred: Tweet => Boolean, accum: TweetSet): TweetSet = {
    val included = if (pred(elem)) accum.incl(elem) else accum

    left.filterAcc(pred, right.filterAcc(pred, included))
  }

  // Why that.incl(elem) union left union right does not work?
  // Why left union (right union that.incl(elem)) is faster than left union right union that incl elem?
  def union(that: TweetSet): TweetSet = left union (right union that.incl(elem))

  def mostRetweeted: Tweet = {
    (left.isEmpty, right.isEmpty) match {
      case (true, true)  => elem
      case (true, false) => List(elem, right.mostRetweeted).maxBy(_.retweets)
      case (false, true) => List(elem, left.mostRetweeted).maxBy(_.retweets)
      case (false, false) =>
        List(elem, right.mostRetweeted, left.mostRetweeted).maxBy(_.retweets)
    }
  }

  def descendingByRetweet: TweetList =
    mostRetweeted :: remove(mostRetweeted).descendingByRetweet
  ...
  def foreach(action: Tweet => Unit): Unit = {
    action(elem)
    left.foreach(action)
...
object Nil extends TweetList {
  ...
  def isEmpty = true
  def ::(elem: Tweet): TweetList = new Cons(elem, Nil)
}

class Cons(val head: Tweet, val tail: TweetList) extends TweetList {
  def isEmpty = false
  def ::(elem: Tweet): TweetList = new Cons(elem, this)
}
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet =
    TweetReader.allTweets.filter(elem => google.exists(elem.text.contains(_)))
  lazy val appleTweets: TweetSet =
    TweetReader.allTweets.filter(elem => apple.exists(elem.text.contains(_)))
