...
object Huffman {
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    ...
    def loop(tree: CodeTree, bits: List[Bit]): List[Char] =
      (bits, tree) match {
        case (_, Leaf(char, _)) => char :: loop(original, bits)
        case (Nil, _)           => Nil
        case (head :: tail, Fork(left, right, _, _)) =>
          if (head == 0) loop(left, tail) else loop(right, tail)
      }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    def loop(tree: CodeTree, text: List[Char]): List[Bit] = {
      (tree, text) match {
        case (_, Nil)                      => Nil
        case (Leaf(char, _), head :: tail) => loop(original, tail)
  ...
  def convert(tree: CodeTree): CodeTable = {
    ...
    def loop(tree: CodeTree, accum: List[Bit]): CodeTable =
      tree match {
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    text.foldLeft(List[Bit]())({ case (bits, char) => bits ::: table(char) })
  }
}
