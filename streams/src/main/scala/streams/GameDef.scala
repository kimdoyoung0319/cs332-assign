...
trait GameDef {
  ...
  /** The case class `Pos` encodes positions in the terrain.
    ...
    *
    * i.e. The position is represented with row-major order.
    *
    * Illustration:
    *
    *     0 1 2 3   <- col.s
  ...
  case class Pos(row: Int, col: Int) {
    ...
    def modifyRow(d: Int) = copy(row = row + d)

    /** The position obtained by changing the `y` coordinate by `d` */
    def modifyCol(d: Int) = copy(col = col + d)
  }
  ...
  case class Block(b1: Pos, b2: Pos) {
    ...
    /** Returns a block where the `row` coordinates of `b1` and `b2` are
      ...
      */
    def modifyRow(d1: Int, d2: Int) = Block(b1.modifyRow(d1), b2.modifyRow(d2))

    /** Returns a block where the `col` coordinates of `b1` and `b2` are
      * changed by `d1` and `d2`, respectively.
      */
    def modifyCol(d1: Int, d2: Int) = Block(b1.modifyCol(d1), b2.modifyCol(d2))

    /** The block obtained by moving left */
    def left = if (isStanding) modifyCol(-2, -1)
    else if (b1.row == b2.row) modifyCol(-1, -2)
    else modifyCol(-1, -1)

    /** The block obtained by moving right */
    def right = if (isStanding) modifyCol(1, 2)
    else if (b1.row == b2.row) modifyCol(2, 1)
    else modifyCol(1, 1)

    /** The block obtained by moving up */
    def up = if (isStanding) modifyRow(-2, -1)
    else if (b1.row == b2.row) modifyRow(-1, -1)
    else modifyRow(-1, -2)

    /** The block obtained by moving down */
    def down = if (isStanding) modifyRow(1, 2)
    else if (b1.row == b2.row) modifyRow(1, 1)
    else modifyRow(2, 1)
